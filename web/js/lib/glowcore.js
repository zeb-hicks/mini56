...
GLOW.Context = (function() {
    ...
    function GLOWContext( parameters ) {
        ...
        this.width                  = parameters.width                 !== undefined ? parameters.width                 : self.innerWidth;
        this.height                 = parameters.height                !== undefined ? parameters.height                : self.innerHeight;
        ...
        if( parameters.context ) {
            ...
            try {
                ...
                if( this.debug && self.WebGLDebugUtils ) {
            ...
            if( this.GL !== null ) {
                ...
                this.enableCulling( true, { frontFace: GL.CW, cullFace: GL.BACK } );
...
GLOW.Compiler = (function() {
    ...
    compiler.defer = function ( parameters, callback ) {
        ...
        callback(new GLOW.CompiledData( program, uniforms, attributes, interleavedAttributes, elements, parameters ));
...
GLOW.FBO = (function() {
    ...
    function GLOWFBO( parameters ) {
        ...
        this.id             = GLOW.uniqueId();
        this.width          = parameters.width  || parameters.size || self.innerWidth;
        this.height         = parameters.height || parameters.size || self.innerHeight;
...
GLOW.Shader = (function() {
    ...
    GLOWShader.prototype.draw = function() {
        ...
        if( cache.active ) {
            ...
            while( a-- ) {
                if (uniformArray[ a ].data !== undefined) {
                    ...
                    if (uniformArray[ a ].undefDrawCount === undefined) {
                        console.warn('Trying to draw an uninitialized shader, uniform %s is undefined.', uniformArray[ a ].name);
                        uniformArray[ a ].undefDrawCount = 1;
                    } else if (++uniformArray [ a ].undefDrawCount % 3000 == 0) {
                        console.warn('Trying to draw an uninitialized shader at least %d times, uniform %s is undefined.', uniformArray [ a ].undefDrawCount, uniformArray[ a ].name);
                    }
                }
            }
        } else {
